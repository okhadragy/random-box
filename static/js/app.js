(function ($) {
  ...
  $(window).on("resize", (e) => {
    ...
    var ow = oldWidth
    var nw = $(window).width()
    var widths = [360, 481, 768, 992, 1200]
    var oldStart = 0
    var oldEnd = 0
    var lastWidth = 0
    var i = 0
    for (const width of widths) {
      if (ow >= lastWidth && ow < width) {
        oldStart = lastWidth
        oldEnd = width
        break
      }
    ...
    if (!(nw >= oldStart && nw < oldEnd && nw<478)) {
      $(".product-details").addClass("d-none")
      $(".product-details").first().removeClass("d-none")
    }
    oldWidth = nw
  })
  ...
  $('.tab').on('click', function () {
    $(".product-details").addClass("d-none")
    $("#" + $(this).attr("id").slice(0, -4) + "-content").removeClass("d-none")
  })

  $(document).on("click", ".slick-arrow", function () {
    $(".product-details").addClass("d-none")
    $("#" + $(".tab[aria-hidden='false']").attr("id").slice(0, -4) + "-content").removeClass("d-none")
  });
